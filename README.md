...
| 模块 | 说明 |
| --- | --- |
| 赛程一览 | 按日期（昨天 / 今天 / 未来 7 天）与赛事分类筛选，覆盖五大联赛（英超、西甲、德甲、意甲、法甲） |
...
| 赔率数据 | The Odds API 低频轮询：每天 09:00 早盘一轮 + 开赛前 15 分钟临场读取（免费档 500 次/月，每日上限 20 次） |
| 数据翻译 | 抓取数据内部翻译后再挂前端：词典翻译天气/位置/风向（离线），MyMemory 在线翻译英文新闻标题（每日限额+缓存） |
...
│   ├── odds-api.js         # The Odds API 赔率（低频轮询：早盘 09:00 + 临场前 15 分钟，配额保护）
│   ├── translate.js        # 数据翻译（词典：天气/位置/风向；MyMemory 在线翻译新闻标题）
